...
public class OperationEditorPaneController implements Initializable {
	...
	public Operation displayDialog(CompteCourant cpte, CategorieOperation mode) {
		...
		case DEBIT:
			...
			break;
		case CREDIT:
			String information = "Cpt. : " + this.compteEdite.idNumCompte + "  "
					+ String.format(Locale.ENGLISH, "%12.02f", this.compteEdite.solde) + "  /  "
					+ String.format(Locale.ENGLISH, "%8d", this.compteEdite.debitAutorise);
			this.lblMessage.setText(information);
			...
			this.cbTypeOpe.setItems(obList);
			this.cbTypeOpe.getSelectionModel().select(0);
			break;
		}
	...
	private void doAjouter() {
		...
		case CREDIT:		
			...
			this.operationResultat = new Operation(-1, montant1, null, null, this.compteEdite.idNumCli, typeOp2);
			this.primaryStage.close();
			break;
		}
	}
